...
void webots_physics_step() {
    ...
    left_foot = dWebotsGetGeomFromDEF("LEFT_FOOT");
    right_foot = dWebotsGetGeomFromDEF("RIGHT_FOOT");
	...
	chosen_time_lift = minimum_time_lift + rnd()*(maximum_time_lift - minimum_time_lift); //we set how long we will lift the robot
		lift_force = 40000.0 * (1.37 - body_Regis_pos[1]) - 10000.0 * body_Regis_vel[1];
		double drageForce = 10;
		//dBodyAddForce (body_Regis, -body_Regis_vel[0]*drageForce, lift_force, -body_Regis_vel[2]*drageForce);
		...
		message_PtoW.self_contact_counter = contact_number-6;
		contact_number = 0;
		// ---------------------------------------------------------------------
